    ...
    public class ClientResponse : ClientMessage
	{
	    ...
	    public void Flush(bool endStream = true)
		{
		    ...
		    if (!string.IsNullOrEmpty(File))
		    {
				...
				{ 
					SetHeader("Content-Type", GetMimeType(File.Substring(File.LastIndexOf('.'))));
					fileHandle = System.IO.File.OpenRead(File);

					SetHeader("Content-Length", fileHandle.Length.ToString());
				}
			}
		    ...
		    if (!Headers.ContainsKey("connection") ||
		        !Headers["Connection"].Equals("upgrade", StringComparison.OrdinalIgnoreCase))
		    {
			    string[] encodings = _request.Headers.ContainsKey("accept-encoding")
				    ? _request.Headers["accept-encoding"]?.Split(',')
				    : null;

			    if (encodings != null && encodings.Contains("gzip", StringComparer.OrdinalIgnoreCase))
			    {
				    const string str = "Content-Encoding: gzip\r\n\r\n";
		    ...
		    if (fileHandle != null)
		    {
				...
				int count;
			    while ((count = fileHandle.Read(buffer, 0, buffer.Length)) > 0)
			    {
		    ...
		    {
			    stream.Write(Encoding.ASCII.GetBytes(responseBody), 0, responseBody.Length);
		    }
